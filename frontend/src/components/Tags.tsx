    ...
    <div>
      ...
      {!preview.value && (
        <div className="mb-4 flex gap-2">
          ...
          {props.onFocused && (
            <button
              onClick={handleTagsCreation}
              className="bg-green-700 dark:bg-green-800 py-2 px-2 rounded-sm text-white cursor-pointer"
            >
              <Check size={16} />
            </button>
          )}
...
const Tag = ({
  ...
  return (
    <div
      className="relative inline-flex items-center px-3 py-1 pr-6 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100 rounded-full text-sm font-medium shadow-sm transition-all cursor-pointer "
      onClick={() => navigate(`/blog/filtered?filter=${title}`)}
    >
      {title}
      {!preview.value && (
        <button
